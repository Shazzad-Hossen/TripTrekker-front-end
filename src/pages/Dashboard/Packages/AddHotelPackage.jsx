...
const AddHotelPackage = ({ details:detailedData }) => {
  ...
  const onSubmit = async (data) => {
    if (data?.division === null || data?.division === undefined)
    ...
    else if (data?.place === null || data?.place === undefined)
      setError("place", { message: "This field is required" });
    else if (Object.keys(photos).length < 5)
      setError("photos", { message: "Upload minimum 5 photos" });
      ...
      if (details) {
        ...
        privatePatch("/api/package", {
          ...data,
          description,
          photos: uploads,
        }).then((res) => {
          res?.status === 200
            ?( toast.success("Successfully Updated"), setDetails(res?.data))
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="flex flex-col gap-2">
              ...
              <div
                className={
                  user?.role === "agency" || user?.role === "hotel"
                    ? "hidden"
                    : "block"
                }
              >
            ...
            <Button
              type="submit"
              className="bg-blue-100 text-white w-fit px-5"
              onClick={() => clearErrors()}
            >
              Add
            </Button>
